...
document.addEventListener('DOMContentLoaded', function () {
    const verifiedListing = 'verified';
    const featuredListing = 'featured';
    const simpleListing = 'simple';
    ...
    function selectCurrency(currency) {
        ...
        fetch(`/api/listing/prices?currency=${currency.id}`)
            .then(response => response.json())
            .then(data => {
                data.prices.forEach(element => {
                    if (element.plan_name == 'Simple Listing') {
                        assign_value_to_plan(element, "simple-listing-price")
                    }
                    else if (element.plan_name == 'Verified Listing') {
                        assign_value_to_plan(element, "verified-listing-price")
                    }
                    if (element.plan_name == 'Featured Listing') {
    ...
    // Code to handle update tool functionality
    ...
    ;
        radioButtons.forEach(radio => {
            radio.addEventListener('change', () => {
                if (radio.checked) {
                    if (radio.value === 'verified'){
                        document.getElementById('updatePrice').innerText = document.getElementsByClassName('verified-listing-price')[0].innerText;
                    }
                    else if (radio.value === 'featured'){
                        document.getElementById('updatePrice').innerText = document.getElementsByClassName('featured-listing-price')[0].innerText;
                    };
                }
            });
        });
    ...
    function handleListingType(listing_type) {
        const radios = document.getElementsByName("inlineRadioOptions");
        const listingType = document.getElementById("listingType");
        const verifiedlistingType = document.getElementById("verified-listing-type");
        const featuredlistingType = document.getElementById("featured-listing-type");


        function togglelistingType() {
            if (document.getElementById("inlineRadio1").checked) {
                listingType.style.display = "block";
                if (listing_type == simpleListing) {
                    verifiedlistingType.style.display = "block";
                    featuredlistingType.style.display = "block";
                    update_listing_type();
                }
                else if (listing_type == verifiedListing) {
                    verifiedlistingType.style.display = "none";
                    featuredlistingType.style.display = "block";
                    update_listing_type();
                }
                else if (listing_type == featuredListing) {
                    listingType.style.display = "none";
                    document.getElementById("inlineRadioforNo").checked = true;
                    showSubscribeAlert('You are already using Featured Listing, Thanks');
                }
            } else {
                listingType.style.display = "none";
                document.getElementById('updatePrice').innerText = '$5';

            }
        }

        radios.forEach(radio => {
            radio.addEventListener("change", togglelistingType);
        });
    ...
    // single tool api calling
    function fetch_tool_info(tool_id){
        fetch(`/api/tools/${tool_id}/`).then(response => response.json()).then(data => {
            if (data.success == true){
                fillup_fields_value(data.tool);
                handleListingType(data.tool.listing_type)               
            }
            else {
                    showSubscribeAlert(`Tool ref ${tool_id} is not exit in our database, Thanks`, 'bg-danger')
                    }
            
        }).catch(error => {
            showSubscribeAlert("Failed to load currency list, try again", "bg-danger");
        });
    }
    ...
    function setupFormSubmissions() {
        // Submit tool form
        const singleToolInfo = document.getElementById('singleToolInfo');
        const submitToolForm = document.getElementById('getToolInfo');
        if (submitToolForm) {
            submitToolForm.addEventListener('submit', (e) => {
                e.preventDefault();
                fetch_tool_info(singleToolInfo.value);
            });
        }
    }

    // fill fields values
    function fillup_fields_value(tool){
        const toolWebsite = document.getElementById('toolWebsite');
        toolWebsite.value = tool.website_url;
        toolWebsite.disabled = true;
    ...
    function showSubscribeAlert(message,t_type, plan='Obtain.AI') {
        ...
        toastEl.innerHTML = `
            <div class="toast-header ${t_type} text-white">
            ...
            <div class="toast-body">
                <p>Thank you for subscribing to our <strong>${plan}</strong></p>
    ...
    if (updatePrice)
    {
    document.getElementById("listingType").style.display = 'none';
